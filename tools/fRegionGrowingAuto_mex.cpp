...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    // 3rd input: The tolerance
    double dTol = double(*mxGetPr(prhs[2]));
    if (dTol < 0.0) {
        dMaxDif = dMax;
    } else {
        dMaxDif = dTol;
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    // 4rd input: The auto-tolerance modifier
    double dMod = 1.0;
    if (nrhs > 3) {
        dMod = double(*mxGetPr(prhs[3]));
    }
    ...
    if (nlhs > 1) {
        const int pSizeOut[] = {1};
        plhs[1] = mxCreateNumericArray(1, pSizeOut, mxDOUBLE_CLASS, mxREAL);
        pdMaxDist = (double*) mxGetData(plhs[1]);
        *pdMaxDist = dMaxDif;
    }
    ...
    if (dDynamicRange == 0.0) {
        for (int iI=0; iI < lImSize; iI++) pbMask[iI] = true;
        return;
    }
    ...
    while (lRegSize < lImSize) {
        ...
        if ((lRegSize == lTestSize) && (dTol < 0.0)) {
            for(int iI = 0; iI < lImSize; iI++) {
                if(pbMask[iI]) dStd += pow(pdImg[iI] - dRegMean, 2);
            }
            dStd = sqrt(dStd/double(lRegSize + 1));
            dMaxDif = 3*dStd*dMod;
            if (nlhs > 1) *pdMaxDist = dMaxDif;
//            mexPrintf("%f %f\n", dRegMean, dStd);
        }
